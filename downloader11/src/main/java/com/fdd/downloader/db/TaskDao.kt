...
interface TaskDao {
     ...
     * Returns: The number of affected rows
     */
    @Query("update ${DownloadDatabase.TABLE_TASK} set ${TaskInfo.COLUMN_STATUS} = :status where ${TaskInfo.COLUMN_FILE_ID} = :fileId")
    fun updateStatusByFileId(fileId: String, status: TaskInfo.TaskStatus): Int
     ...
     * 根据当前状态更新下载状态
     */
    @Query("update ${DownloadDatabase.TABLE_TASK} set ${TaskInfo.COLUMN_STATUS} = :update where ${TaskInfo.COLUMN_FILE_ID} = :fileId and ${TaskInfo.COLUMN_STATUS} in (:expect)")
    fun compareAndUpdateStatusByFileId(fileId: String, update: TaskInfo.TaskStatus, expect: List<TaskInfo.TaskStatus>): Int
     ...
     * 根据 fileId 查询下载任务
     */
    @Query("select * from ${DownloadDatabase.TABLE_TASK} where ${TaskInfo.COLUMN_FILE_ID} =:fileId")
    fun queryByFileId(fileId: String): TaskInfo?

...
import com.fdd.downloader.TaskInfo.Companion.COLUMN_IDENTITY
 ...
 *
 */
@Entity(tableName = DownloadDatabase.TABLE_TASK, indices = [Index(value = [COLUMN_IDENTITY], unique = true)])
class TaskInfo {
    ...
    var id: Long = 0L                               //数据库中的 id

    @ColumnInfo(name = COLUMN_IDENTITY)
    var identity: String = ""                         //文件唯一 id
    ...
    class Builder {
        ...
        fun build(): TaskInfo {
            if (url.isEmpty()) {
                throw RuntimeException("download url is empty!!!")
            }
            if (identity.isEmpty()) {
                identity = url
            }

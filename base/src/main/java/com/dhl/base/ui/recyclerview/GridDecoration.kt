...
class GridDecoration(
    ...
    override fun getItemOffsets(outRect: Rect, view: View, parent: RecyclerView, state: RecyclerView.State) {
        ...
        outRect.right = itemHorPadding - (column + 1) * itemHorPadding / span; // 列间距 - (column + 1) * (列间距 * (1f /列数))

        val row = if (state.itemCount % span == 0) state.itemCount / span else state.itemCount / span + 1
        when {
            position < span -> {//第一行
